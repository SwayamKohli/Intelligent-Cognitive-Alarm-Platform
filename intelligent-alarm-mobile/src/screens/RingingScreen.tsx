...
export default function RingingScreen({ route, navigation }: any) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const { success, dismiss_alarm, current_streak, target_streak } =
        response.data;
      ...
      if (success) {
        if (dismiss_alarm) {
          ...
          setFeedback({
            type: "success",
            message: `Correct! Streak ${current_streak}/${target_streak}`,
          });
          fetchChallenge();
        }
      } else {
  ...
  const handleSnooze = async () => {
    try {
      ...
      if (error.response?.status === 400) {
        setFeedback({
          type: "error",
          message: "Snooze limit reached — solve the challenge now.",
        });
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        style={[
          styles.verifyButton,
          (submitting || loading) && styles.disabledButton,
        ]}
