...
async def export_pdf_report(
    ...
    habit_score_data = [
        ...
        [
            Paragraph("Consistency Score", cell_normal),
            Paragraph(f"{consistency}%", cell_normal),
            Paragraph(f"{days_active} of 7 active days logged.", cell_normal),
        ],
        [
            Paragraph("Challenge Success Rate", cell_normal),
            Paragraph(f"{challenge_rate}%", cell_normal),
            Paragraph(f"Failure rate: {failure_rate}%.", cell_normal),
        ],
        [
            Paragraph("Snooze Reduction Rate", cell_normal),
            Paragraph(f"{snooze_reduction}%", cell_normal),
            Paragraph(f"Total snoozes this week: {total_snoozes}.", cell_normal),
        ],
        [
            Paragraph("Sleep Schedule Adherence", cell_normal),
            Paragraph(f"{sleep_adherence}%", cell_normal),
            Paragraph("Schedule configuration adherence status.", cell_normal),
        ],
    ]
    ...
    telemetry_data_table = [
        ...
        [
            Paragraph("Active Days (Last 7 Days)", cell_normal),
            Paragraph(f"{days_active} days", cell_normal),
        ],
        [
            Paragraph("Total Snoozes Triggered", cell_normal),
            Paragraph(f"{total_snoozes} times", cell_normal),
        ],
        [
            Paragraph("Total Cognitive Challenges Attempted", cell_normal),
            Paragraph(f"{telemetry.get('total_challenges', 0)} attempts", cell_normal),
        ],
        [
            Paragraph("Total Challenge Failures", cell_normal),
            Paragraph(f"{telemetry.get('total_failures', 0)} failures", cell_normal),
        ],
        [
            Paragraph("Challenge Failure Percentage", cell_normal),
            Paragraph(f"{failure_rate}%", cell_normal),
        ],
    ]
...
async def export_excel_report(
    ...
    if not raw_logs:
        df_telemetry = pd.DataFrame(
            [
                ...
                {
                    "Summary Metric": "7-Day Aggregated Challenges",
                    "Value": telemetry.get("total_challenges", 0),
                },
    ...
    df_habits = (
        pd.DataFrame(habit_list_data)
        if habit_list_data
        else pd.DataFrame([{"Message": "No active habits configured."}])
    )
    df_habit_logs = (
        pd.DataFrame(habit_log_data)
        if habit_log_data
        else pd.DataFrame([{"Message": "No habit logs recorded."}])
    )
